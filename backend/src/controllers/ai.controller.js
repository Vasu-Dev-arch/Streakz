 ...
 * Body: { goal: string }
 *
 * Calls the Groq API and returns a structured habit plan:
 * {
 *   goal: string,
 *   habits: [{ name, emoji, why, frequency }],
 *   tip: string
 * }
 */
export async function habitCoach(req, res) {
  const { goal } = req.body || {};
...
- Do not include any text outside the JSON object`;
  ...
  const userMessage = `My goal: ${trimmedGoal}`;
  ...
  // Validate structure so the frontend can rely on the shape
  if (
